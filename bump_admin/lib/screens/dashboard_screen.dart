...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> _loadMaintenance() async {
    try {
      final data = await supabase.from('system_config').select('value').eq('key', 'maintenance').single();
  ...
  Future<void> _toggleMaintenance() async {
    if (!_maintenanceEnabled) {
      ...
      if (confirm == true) {
        try {
          await supabase.from('system_config').update({'value': {'enabled': true, 'message': msgCtrl.text.trim(), 'estimated_time': timeCtrl.text.trim()}, 'updated_at': DateTime.now().toIso8601String()}).eq('key', 'maintenance');
      ...
      if (confirm == true) {
        try {
          await supabase.from('system_config').update({'value': {'enabled': false, 'message': '', 'estimated_time': ''}, 'updated_at': DateTime.now().toIso8601String()}).eq('key', 'maintenance');
  ...
  Future<void> _loadStats() async {
    try {
      final userRes = await supabase.from('users').select('id');
      final pendingRes = await supabase.from('reports').select('id').eq('status', 'pending');
      final totalRes = await supabase.from('reports').select('id');
