  ...
  Future<void> _deleteUser(String userId, String name) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Bạn có chắc muốn xóa người dùng "$name"?', style: const TextStyle(color: Color(0xFF9CA3AF))),
        actions: [
    ...
    if (confirm == true) {
      try {
        // 1. Xóa khỏi bảng public.users
        await supabase.from('users').delete().eq('id', userId);

        // 2. Xóa khỏi auth.users (Supabase Admin API)
        ...
        if (response.statusCode != 200 && response.statusCode != 204) {
          debugPrint('Lỗi xóa auth user: ${response.statusCode} ${response.body}');
        }
        ...
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Đã xóa người dùng hoàn toàn'), backgroundColor: Colors.green));
      } catch (e) {
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return _UserTile(
                            ...
                            onDelete: () => _deleteUser(user['id'], user['name'] ?? 'Unknown'),
                          );
                        },
                      ),
                    ),
...
class _UserTile extends StatelessWidget {
  ...
  const _UserTile({required this.user, required this.onTap, required this.onDelete});
