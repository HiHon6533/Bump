...
import { getSharedPhotos, updateNickname, toggleMute } from '../../../services/chatService';
...
export default function ChatSettingsScreen() {
    ...
    try {
      ...
      if (conv) {
        ...
        if (!resolvedOtherUserId) {
          ...
          if (otherId) setResolvedOtherUserId(otherId);
        }
      }
    } catch (e) {
        ...
        { text: 'Hủy', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Đã xóa', 'Lịch sử chat đã được xóa.');
            } catch (e) {
  ...
  // ---- Setting Row ----
  const SettingRow = ({ icon, label, iconBg, onPress, trailing, borderBottom = true }: {
    icon: string; label: string; iconBg: string;
    onPress?: () => void; trailing?: React.ReactNode; borderBottom?: boolean;
  }) => (
    <TouchableOpacity
